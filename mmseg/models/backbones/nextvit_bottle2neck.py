...
class NextViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
        ...
        # Next Hybrid Strategy
        self.stage_block_types = [([NCB] * (depths[0]-1))+[Bottle2neck],
                                  # [NCB] * (depths[0] - 1)+[Bottle2neck],
                                  [NCB] * (depths[1] - 2)+[Bottle2neck]+ [NTB],
                                  [NCB, NCB, NCB, NCB, NTB] * (depths[2] // 5),
                                  [NCB] * (depths[3] - 1) + [NTB]]
        ...
        for stage_id in range(len(depths)):
            ...
            for block_id in range(numrepeat):
                ...
                if block_type is NCB:
                    ...
                    features.append(layer)
                elif block_type is Bottle2neck:
                    layer = Bottle2neck(input_channel, output_channel, stride=1,downsample=None)
                    features.append(layer)
...
class nextvit_bottle2neck(NextViT):
    def __init__(self, resume='', **kwargs):
        super(nextvit_bottle2neck, self).__init__(
